...
<?php foreach ($posts as $p) : ?>
  <article class="card">
    ...
    <div class="card__actions">
      ...
      <button class="btn btn--ghost open-edit" type="button" data-id="<?= (int)$p['id'] ?>"  data-person="<?= (int)$p['person_base_id'] ?>" data-content="<?= htmlspecialchars($p['content'] ?? '', ENT_QUOTES, 'UTF-8') ?>" data-date="<?= htmlspecialchars($p['post_date'] ?? '', ENT_QUOTES, 'UTF-8') ?>">Edit</button>
